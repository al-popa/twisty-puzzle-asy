...
transform3 t = rotate(-20, AJ);
transform3 p = rotate(-20, AX);
...
face ff = constructFace(r * AF, f * (AE + AF + AT), conf5);
face fg = constructFace(r * AG, f * (AG + AH + AI), conf5);
face fh = constructFace(r * AH, f * (AF + AH + AU), conf5);
face fi = constructFace(r * AI, f * (AB + AI + AK), conf5);
face fj = constructFace(r * AJ, f * (AH + AI + AJ), conf5);
face fk = constructFace(r * AK, f * (AK + AL + AM), conf5);
face fl = constructFace(r * AL, f * (AJ + AL + AV), conf5);
face fm = constructFace(r * AM, f * (AC + AM + AO), conf5);
face fn = constructFace(r * AN, f * (AL + AM + AN), conf5);
face fo = constructFace(r * AO, f * (AO + AP + AQ), conf5);
face fp = constructFace(r * AP, f * (AN + AP + AW), conf5);
face fq = constructFace(r * AQ, f * (AD + AQ + AS), conf5);
face fr = constructFace(r * AR, f * (AP + AQ + AR), conf5);
face fs = constructFace(r * AS, f * (AE + AS + AT), conf5);
face ft = constructFace(r * AT, f * (AR + AT + AX), conf5);
face fu = constructFace(r * AU, f * (AF + AH + AU), conf4);
face fv = constructFace(r * AV, f * (AJ + AL + AV), conf4);
face fw = constructFace(r * AW, f * (AN + AP + AW), conf4);
face fx = constructFace(r * AX, f * (AR + AT + AX), conf4);

real g = length(fa.R[0] - fa.P[0]) / 2;
...
glueFaces(ff, fh, fu, g);
glueFaces(fj, fl, fv, g);
glueFaces(fn, fp, fw, g);
glueFaces(fr, ft, fx, g);
glueFaces(fa, fb, g);
glueFaces(fb, fc, g);
glueFaces(fc, fd, g);
glueFaces(fd, fa, g);
...
vertex4 vzza = constructVertex4(fx, fr, fp, fw);
vertex4 vzzb = constructVertex4(fw, fn, fl, fv);
vertex4 vzzc = constructVertex4(fv, fj, fh, fu);
vertex4 vzzd = constructVertex4(fu, ff, ft, fx);

drawFace(fa, RGB(121, 45, 75)); // Bordo
drawFace(fb, RGB(255, 106, 3)); // Orange
drawFace(fc, RGB(196, 0, 66)); // Red
drawFace(fd, RGB(206, 115, 24)); // Coffee
drawFace(fe, RGB(164, 174, 173)); // Grey
drawFace(ff, RGB(233, 211, 174)); // Beige
drawFace(fg, RGB(255, 228, 1)); // Yellow
drawFace(fh, RGB(255, 250, 200), t, 4); // Pale Yellow
drawFace(fi, RGB(235, 175, 1), t, 8); // Signal Yellow
drawFace(fj, RGB(230, 252, 129), t); // Tea Yellow
drawFace(fk, RGB(1, 156, 220), t, 8); // Light Blue
drawFace(fl, RGB(14, 4, 126), t, 10); // Dark Blue
...
drawFace(fq, RGB(177, 149, 198)); // Lilac
drawFace(fr, RGB(246, 166, 191), p, 4); // Pink
drawFace(fs, RGB(77, 37, 1)); // Chocolate
drawFace(ft, RGB(255, 255, 255), p, 1); // White
drawFace(fu, RGB(90, 173, 65), p, 4); // Mint Green
drawFace(fv, RGB(1, 88, 54), t, 1); // Grass Green
drawFace(fw, RGB(0, 173, 169), p, 7); // Turquase
drawFace(fx, RGB(0, 147, 102), p); // Green

drawVertex4(vz, RGB(0, 147, 102)); // Green
drawVertex4(vza, RGB(0, 173, 169)); // Turquase
drawVertex4(vzb, RGB(255, 255, 255)); // White
drawVertex4(vzc, RGB(255, 250, 200)); // Pale Yellow
drawVertex4(vzd, RGB(14, 4, 126)); // Dark Blue
drawVertex4(vzz, RGB(196, 0, 66), p, 0); // Red
drawVertex4(vzza, RGB(121, 45, 75), p, 0); // Bordo
drawVertex4(vzzb, RGB(177, 149, 198)); // Lilac
drawVertex4(vzzc, RGB(55, 58, 191), t, 1); // Brilliant Blue
drawVertex4(vzzd, RGB(255, 106, 3), p, 3); // Orange
